...
if (isset($_SESSION["idAdmin"])) {
    if (!isset($_GET["id"]) || $_GET["id"] == "") {
        header("location:../../connexion/connexion-admin.php");
          ...
          $countProduct = $dbRequest->query("SELECT * FROM `products` WHERE id=$id AND categorie_id=$nbrCat");
          $productInfo  = $countProduct->fetchAll(PDO::FETCH_OBJ);

          $countProductDesc = $dbRequest->query("SELECT * FROM `description_laptop` WHERE id_product=$id");
          $productInfoDesc   = $countProductDesc->fetchAll(PDO::FETCH_OBJ);
...
<a href="connexion.php">
    ...
    <h1 class="mt-4 title" align="center">Update product</h1>
                     ...
                     <div class="col-md-6">
                        <div class="card-custom form-container shadow-sm w-100">
                                 <form method="post" id="form-laptop">
                                       <input type="text" name="lib_product_lap" placeholder="Nom complet du produit" class="form-control form-custom" value="<?= $productInfo[0]->lib_product?>">
                                       <input type="number" name="price_product_lap" class="form-control mt-2 form-custom" placeholder="Prix du produit" value="<?= $productInfo[0]->price?>">
                                       <input type="hidden" name="choice_laptop" value="update">
                                       <input type="hidden" name="id_get" value="<?= $_GET["id"] ?>">
                                 </form>
                        </div>
                     </div>
                     <div class="col-md-6">
                       ...
                       <div class="card-custom  form-container shadow-sm w-100 mt-4 p-4">
                             <div class="row">
                                   <div class="col-md-6">
                                             ...
                                             <input type="text" class="form-control form-custom mt-2" placeholder="Memoire Graphique" name="graphique_lap" form="form-laptop" value="<?= $productInfoDesc[0]->graphique_product?>"> 
                                             <input type="text" class="form-control form-custom mt-2" placeholder="Ecran" name="ecran_lap" form="form-laptop"  value="<?= $productInfoDesc[0]->screen_product?>"> 
                                             <input type="text" class="form-control form-custom mt-2" placeholder="RAM" name="ram_lap" form="form-laptop"  value="<?= $productInfoDesc[0]->ram_product?>"> 
                                             <input type="text" class="form-control form-custom mt-2" placeholder="Interne" name="interne_lap" form="form-laptop"  value="<?= $productInfoDesc[0]->rom_product?>">
                                             <input type="text" class="form-control form-custom mt-2" placeholder="Processeur" name="processeur_lap" form="form-laptop"  value="<?= $productInfoDesc[0]->processor_product?>"> 
                                  

                                   </div>
                                   <div class="col-md-6">
                                            <textarea name="other_desc_product_pc" id="" cols="30" rows="13" class="form-control form-custom" placeholder="<?= $productInfoDesc[0]->other_description?>" form="form-laptop"></textarea>
                                            
                                   </div>
                                   <div class="p-2 mt-2">
                                          <button type="submit" class="w-100 btn btn-primary btn_laptop_submit" form="form-laptop" id="button-laptop">Update product</button>
                                   </div>

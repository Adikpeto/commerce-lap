...
if (!isset($_GET["search"]) && !isset($_GET["page"])) {
  header("location:laptop.php?page=1");
}
...
$compteR = $dbRequest->query("SELECT count(id) FROM `products` WHERE categorie_id=1 LIMIT 4 OFFSET 0");
...
if (($_GET["page"] > $nbrPages) || ($_GET["page"]  < 1)) {
    ...
    for ($i=1; $i <= $nbrPages; $i++) { 
        ...
        if ($_GET["page"] == $i) {
            ...
            $ligne4 = ($debut + 12);
            $dbRequest = $DB->PDOConnecte();
            $productLigne1R = $dbRequest->query("SELECT * FROM `products` WHERE categorie_id=1 LIMIT 4 OFFSET $debut");
            $productLigne1 = $productLigne1R->fetchAll(PDO::FETCH_OBJ);
    
    
            $productLigne2R = $dbRequest->query("SELECT * FROM `products` WHERE categorie_id=1 LIMIT 4 OFFSET $ligne2");
            $productLigne2 = $productLigne2R->fetchAll(PDO::FETCH_OBJ);
    
            $productLigne3R = $dbRequest->query("SELECT * FROM `products` WHERE categorie_id=1 LIMIT 4 OFFSET $ligne3");
            $productLigne3 = $productLigne3R->fetchAll(PDO::FETCH_OBJ);
    
            $productLigne4R = $dbRequest->query("SELECT * FROM `products` WHERE categorie_id=1 LIMIT 4 OFFSET $ligne4");
...
if (isset($_GET["search"])) {
  ...
  $searchRequest = $dbSearchRequest->query("SELECT * FROM `products` WHERE categorie_id=1 AND lib_product LIKE '%$keyWord%'");
...
<body>
      ...
      <div class="col-md-12">
        <div class="row justify-content-center">
          <div class="col-md-12 p-4">
            ...
            <?php if ($search == false) :?>
              <div class="small-title-top mb-2">Nous vendons plus de 1000 laptop sur notre site.</div>
            <?php endif?>
            <?php if ($search == true) :?>
                ...
                <div class="product_card_clone">
                  ...
                  <div class="product_img_card_clone">
                    <div class="product_img_clone"
                      ...
                      <div class="product_info_plus_clone">
  ...
  <div class="container">
    <div class="pargination-container">
      <?php  $color = "blue";$colorText="white" ?>
